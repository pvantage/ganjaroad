            ...
            {!! Form::open(array('url' => URL::to('newclassified'), 'method' => 'post', 'id' => 'classified', 'class' => 'bf', 'files'=> true)) !!}
				 <div class="row">
                    <div class="col-sm-12">
								...
								<div class="dropdowns">
									@if($cities = Form::old('multicategory'))
										...
										@foreach($cities as $city)
											@php
												if(!isset($options[$city])) continue;
											@endphp
						...
						<?php /* maybe we will go back to this, for now required
						@if(TemplateHelper::isDeveloper())
							<div class="form-group">
								<div class="row">
									<div class="col-md-6">
										{!! Form::label('recurring', trans('classified/form.recurring')) !!}
									</div>
									<div class="col-md-6">
										{!! Form::YesNo('recurring') !!}
									</div>
								</div>
							</div>
						@endif
							...
							<div class="form-group recurring {{ $errors->first('recurring', 'has-error') }}">
								<div class="row">
									<div class="col-md-12 recurring-box">
                                        @lang('classified/form.recurring_checkbox_all_ads_are_moved')

                                        <label>
											{!! Form::checkbox('recurring', 1, true) !!}

											@lang(
                                                'classified/form.recurring_checkbox',
                                                ['default' => Lang::get('classified/form.renewal_every_2_weeks')]
											)
										</label>

										<span class="help-block">
                                            {{ $errors->first('recurring', ':message') }}
                                        </span>
									</div>
								</div>
							</div>
...
jQuery( document ).ready(function() {
	...
	jQuery("input[name='renewal']").on('ifClicked', function (event) {
		jQuery('.renevalperiod-text').text(jQuery(this).closest('li').text());
	});
...
//	};
	...
	myDropzone.on('queuecomplete', function () {
		$('form#classified')[0].submit();
	});
	
	jQuery('form#classified').submit(function( event ) {
		event.preventDefault();

		if ($('#previews').find('.file-row').length) {
			myDropzone.enqueueFiles(myDropzone.getFilesWithStatus(Dropzone.ADDED));
		} else {
			$(this)[0].submit();
		}
			...
			if(data.success && data.max) {	
				...
				if(data.tier) {
					...
					jQuery(data.tier).each(function(index, option) {
						if(option.to > 100) {
							tier_html += '<div class="col-md-12">' + option.formated_price + ' Every 2 Weeks for ' + option.from +'+ @lang('tier/form.per_each_ad')</div>';
						} else {
							tier_html += '<div class="col-md-12">' + option.formated_price + ' Every 2 Weeks for ' + option.from + '-' + option.to + ' @lang('tier/form.per_each_ad')</div>';
						}						
					});
					tier_html += '</div>';
